...
# Y = X + tau Eta
# X_0 ~ N(0, 1)
# parameters[0, :] = sigma^2 > 0
# parameters[1, :] = tau^2 > 0
################################################################
...
def transitionAndWeight(states, y, parameters, t):
    ...
    """
    y = array([y])
    Nx = states.shape[0] #state is defined in ParallelSIR by e.g. self.xparticles = zeros((self.Nx, self.statedimension, self.Ntheta))
...
def computeHscore(states, y, parameters, xweights, thetaweights, t):
    ...
    for (int i = 0; i < Ntheta; i++)
    {
        for (int j = 0; j < Nx; j++)
        {
            temp = - 1 / parameters(1, i) * ((double) y(0) - states(j, 0, i));
            grad(i) = grad(i)  + temp * xweights(j, i);
            lap(i) = lap(i) + ( - 1 / parameters(1, i) + temp * temp ) * xweights(j, i);
        }
