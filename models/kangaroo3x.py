...
# state: N_t2 = N_t1 exp(sigma RW(t2-t1))
# observ: 
# params[0]: tau
# params[1]: sigma
...
# Time_start = 0
# Time = [float(x)/3 for x in range(1,3000)]
...
def observationGenerator(states, parameters): 
    #input states is 2D: numStateParticles x dimObs, parameters is 1D
    ...
    return obs
    # p = 1 / (1 + parameters[0] * states[i,0])
    # n = floor( states[i,0] * p / (1-p) )
    # return nbinom.rvs(n, p, size = states.shape[0])

    #the following funcion is a key: it sample the last states, and output new states & (loglik) weights (filtering density)
...
modelx.setParameters(array([0.05, 0.10])) #put true params here to generate synthetic data, ow. COMMENT and choose a datafile in userfile.py 
#modelx.setParameters(array([0.5, 1])) #converges well only for the second param in synthetic data experiment 
