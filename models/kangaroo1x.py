...
# state: t1 to t2: d Nt / Nt = (r + sigma^2 / 2 - b Nt) dt + sigma d Wt
# the process of Yt = log(Nt) satisfies d Yt = (r-b e^Yt) dt + sigma d Wt
# observ: 
# params[0]: tau
# params[1]: sigma
# params[2]: r
...
def observationGenerator(states, parameters): 
    #input states is 2D: numStates x dimStates, parameters is 1D
    ...
    for i in range(states.shape[0]):
        ...
        p = minimum(p, 1-1e-7) 
        p = maximum(p, 1e-7) #to prevent overflow  
...
def transitionAndWeight(states, y, parameters, t): #here, the last argument t now becomes useful 
 ...
 #   print time 
    ...
    """ % {"time": time, "HScoreInference": HScoreInference, "Del": Del, "sqrtDel": sqrtDel}
    ...
    if time > 0: #update state particles
        nInter = int(floor(time / Del)) #num of interpolating points 
        # print '\n', 'nInter=', nInter
        noise = random.multivariate_normal(repeat(0,1), [[1]], (nInter, Nx, Ntheta)) #dim: Nx x Ntheta x M
        weave.inline(code,['Nx', 'Ntheta', 'Dx', 'Dparam', 'states', 'parameters', 'noise', 'weights', \
            'simpleHScore', 'grad', 'lap', 'weightsNormalized', 'time', 'nInter'], \
            type_converters=weave.converters.blitz, libraries = ["m"])
    # for j in range(Ntheta):
    #     for k in range(Nx):
    #         p = 1 / (1 + parameters[0,j] * states[k,0,j])
    #         p = min(p, 1-1e-4)
    #         n = int( max(1, floor(states[k,0,j] * p / (1-p)) ) )
    #         weights[k,j] = nbinom.logpmf(y0, n, p) + nbinom.logpmf(y1, n, p) 
    # for k in range(Nx):
...
#     time = Time[t]-Time[0] #not yet used for this function
#     code = \
#     """ 
#     double temp;
#     double ave_grad = 0;
...
modelx.setParameters(array([0.06, 0.7, 1.7, 0.003])) #put true params here to generate synthetic data, ow. COMMENT and choose a datafile in userfile.py 
