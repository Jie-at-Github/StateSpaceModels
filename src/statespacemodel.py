...
class SSM:
    def __init__(self, name = "", xdimension = 1, ydimension = 1, HScoreInference = 0, continuousObs = 0):
        print 'creating state space model "%s"' % name
        self.name = name
        self.xdimension = xdimension
        self.ydimension = ydimension
        self.HScoreInference = HScoreInference
        self.continuousObs = continuousObs
    ...
    def setComputeHscore(self, function):
        self.computeHscore = function
...
def predtmp%(idim)i(xparticles, thetaweights, thetaparticles, t):
    ...
    def generateData(self, size, parameters, savefilename):
        ...
        for t in xrange(0, size):
            ...
            TaWresults = self.transitionAndWeight(previousstate[newaxis, :, newaxis], y = zeros(1), \
                    parameters = parameters[:, newaxis], t = t) #TandW function is used here only for convenience (for generating states)
