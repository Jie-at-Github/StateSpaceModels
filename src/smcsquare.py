...
class SMCsquare:
    ...
    def __init__(self, model, algorithmparameters, \
        ...
        self.lScore = zeros(self.T)
        self.hScore = zeros(self.T)
        #below is for computing the hscore of discrete obs. 
        self.thetaparticles_last = zeros((self.modeltheta.parameterdimension, self.Ntheta))
        self.logxweights_last = zeros((self.Nx, self.Ntheta))
        self.thetalogweights_last = zeros(self.Ntheta)
    ...
    def first_step(self):
        ...
        self.priordensityeval = apply_along_axis(func1d = self.modeltheta.priorlogdensity,\
                arr = self.transformedthetaparticles, axis = 0)
        #also init. the quantities for disc-obs. H score 
        self.thetaparticles_last = self.thetaparticles #keep self.logxweights_last self.thetalogweights_last zero arrays
    ...
    #insert computation of H score (not cumulated), for hiddenAR model
    def getHScore(self, t):
        yt = self.observations[t] 
        Nx = self.Nx
        Ntheta = self.Ntheta
        paramDim= self.thetaparticles.shape[0]
        grad = zeros(Ntheta) #zeros((Nx, Ntheta))
        lap = zeros(Ntheta)
        #xNormConst = sum(self.xweights, axis=0) CANNOT USE xweight because it is set zero in PMCMC enlarging step 
            #(causing zero demonimator in lap and grad calculations)
        xweights_safe = exp(self.logxweights)        
        xNormConst = sum(xweights_safe, axis=0)
        thetaweights = exp(self.thetalogweights[t, :])
        thetaweights = thetaweights / sum(thetaweights)
        hScore_theta = zeros(Ntheta)
        
        modelName = "nestedLGSSM"
        
        if modelName == "hiddenAR":
            TAU = 0.5 #sd
            for i in range(Ntheta):
                grad[i] = 0
                lap[i] = 0
                for j in range(Nx):
                    temp = (yt - self.xparticles[j,0,i]) / (TAU**2)
                    grad[i] += - temp * xweights_safe[j,i]
                    lap[i] += ( -1 / (TAU**2) + temp ** 2 ) * xweights_safe[j,i]
                grad[i] /= xNormConst[i]
                lap[i] = -grad[i]*grad[i] + lap[i] / xNormConst[i]             
                hScore_theta[i] = lap[i] + (grad[i] ** 2) / 2
                
        if modelName == "nestedLGSSM":   
            R = 0.5 #var 
            for i in range(Ntheta):
                grad[i] = 0
                lap[i] = 0
                for j in range(Nx):
                    temp = ( yt - dot(self.xparticles[j,0:paramDim-1,i], self.thetaparticles[:,i]) ) / R
                    grad[i] += - temp * xweights_safe[j,i]
                    lap[i] += ( -1 / R + temp ** 2 ) * xweights_safe[j,i]
                grad[i] /= xNormConst[i]
                lap[i] = -grad[i]*grad[i] + lap[i] / xNormConst[i]             
                hScore_theta[i] = lap[i] + (grad[i] ** 2) / 2
                
        hScore = average(hScore_theta, weights = thetaweights)
        ave_score = average(grad, weights = thetaweights)
        hScore += 0.5 * average( (grad - ave_score) ** 2, weights = thetaweights)
        return hScore
    ...
    def getResults(self):
        ...
        resultsDict = {"Ntheta" : self.Ntheta, "Nx" : self.Nx, "T": self.T, \
                ...
                "evidences": self.evidences, "lScore": self.lScore, "hScore": self.hScore, \
                "smoothedmeans": self.smoothedmeans, \
                "smoothedvalues": self.smoothedvalues, \
                "computingtimes": self.computingtimes, "data": self.observations}
